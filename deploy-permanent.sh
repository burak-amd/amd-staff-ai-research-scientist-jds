#!/bin/bash
# Deploy amd-staff-ai-research-scientist-share to GitHub Pages (burak-amd account).
set -euo pipefail

DECK="$(cd "$(dirname "$0")" && pwd)"
REPO_NAME="amd-staff-ai-research-scientist-jds"
...
if [ -z "$(git status --porcelain 2>/dev/null)" ] && [ -n "$(git rev-parse HEAD 2>/dev/null)" ]; then
  ...
  git add index.html optimization.html rl.html .nojekyll SHARE.md deploy-permanent.sh
  git commit -m "Add Staff AI Research Scientist job descriptions (two variants)"
else
  git add -A
  git diff --cached --quiet || git commit -m "Update Staff AI Research Scientist job descriptions"
fi
